...
Lifecycle:
- [x] Uploads freeze 7 days after the event date (cron; hardcoded date from issue 03) —
      the existing manual admin toggle remains as an override
- [x] Freeze fires both zip builds; no rebuild on later gallery changes (post-freeze
      deletions/hides are not re-packed)
- [x] Zip layout per MVP issue 15: folder per uploader display name, filenames prefixed
      with taken-timestamp; store-only (no compression)
- [x] Job table row per zip: state `packing|ready|failed`, progress N of M; build runs
      chunked/resumable if one invocation can't finish (S3 multipart to Supabase Storage)

Stable-link flow (no rotting URLs):
- [x] Zip objects are permanent files in a private bucket
- [x] `GET /api/export/public` (open) and `GET /api/export/admin` (admin session): if job
      `ready` → mint fresh ~24h signed URL → 302; if `packing` → "preparing" state, never 404
- [x] "Копирај линк" copies the stable endpoint URL, never the signed URL
- [x] Admin zip only reachable behind admin session

UI:
- [x] 13a sheet: summary rows (count · оригинали · size); guests see no "Приватне" row;
      admin sheet is informational (both zips always build — no Укључи/Изостави toggle)
- [x] 13b running card (6b geometry) with progress, re-hydrates from job row across visits
- [x] 13c ready card: "ZIP је спреман · size", "Преузми сада" + "Копирај линк"; drop the
...
Verified 2026-08-28: resumable build uses Supabase's tus endpoint, not S3 multipart —
same chunked/resumable behavior, different mechanism.
