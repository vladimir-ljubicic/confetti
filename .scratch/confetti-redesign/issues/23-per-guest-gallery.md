...
- [ ] Compact bar always visible (no ceremonial masthead): guest's 34px avatar (initial
      style as in the viewer pill), name at 19px Cormorant,
      "12 фотографија · 41 ♥" (11px, letter-spacing 0.16em)
- [ ] Stats computed server-side over the guest's public photos: count + sum of
      `like_count`. Serbian pluralization for фотографија/фотографије/фотографија
- [ ] No upload button
- [ ] Private photos never appear (verify `loadPublicPhotos` already excludes them here)
- [ ] Viewer opened here scopes swiping to this guest's photos, counter "3 / 12" —
