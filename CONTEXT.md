...
**Export target**:
Whose photos a ZIP holds. Two are shared, packed once for everyone: the public gallery, and the admin's — public photos, plus private ones when the admin asks for them. The third is one guest's: every photo their device uploaded, private ones with it. There is one per guest, and only the couple reach it, from that guest's admin page; the guest already holds those photos on their device.

**Export job**:
The server-side packing of one ZIP per export target. Preparing it starts the job or restarts a cancelled or expired one; it packs until ready, and every ZIP but the public one can be cancelled while it packs. The public ZIP can only be prepared once frozen; the admin's and a guest's at any time. The freeze replaces a shared ZIP whose snapshot was taken while uploads were open and leaves a guest's alone — it exists only once the couple ask for it. The private-photos choice is part of the snapshot: preparing with the other choice replaces the live admin ZIP.
