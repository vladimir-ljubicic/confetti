...
import { saveUploaderProfile } from "@/lib/uploaders";
...
export async function POST(request: Request) {
  ...
  const saved = await saveUploaderProfile(deviceId, profile).catch((error) => {
    console.error("Saving profile failed", error);
    return null;
  });
  if (!saved) return jsonError("Could not save profile", 500);
