    ...
    <form action={formAction} className="flex w-full max-w-xs flex-col gap-3">
      <label className="flex flex-col gap-1 text-sm text-ink">
        ...
        <input
          ...
          required
          autoFocus
          className="rounded-lg border border-ink/20 bg-white px-3 py-2 text-base outline-none focus:border-gold-deep"
        />
      </label>
      <button
        type="submit"
        disabled={pending}
        className="rounded-lg bg-gold-deep px-4 py-2 text-sm font-medium text-white transition hover:opacity-90 disabled:opacity-60"
      >
        {labels.submit}
      </button>
