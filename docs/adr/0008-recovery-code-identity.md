...
hide, publish or delete their own photos, with their name still on
...
other off the count) and its now-empty uploader row goes. The zip packed for
it goes with it, object and all: the photos it packed have changed hands, so
it snapshots a gallery that is no longer that guest's. Merging and counting
share one transaction in `redeem_recovery_code`, so a redemption cannot
half-happen.
...
- A code is a bearer credential: whoever types it becomes that guest, with
  their private photos. Six characters is 32^6 ≈ 1.07e9, and a wedding-sized
  album puts a couple of hundred codes in it, so a blind guess lands about
  twice in ten million. Wrong guesses are counted per device in
